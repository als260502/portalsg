 ...
 */!function (e) {
    ...
    h.prototype = {constructor: h, init: function (e) {
            var i = this;
            ...
            var i = this, t = "", a = i.initialPreview, n = i.initialPreviewCount, r = i.initialCaption.length, l = "preview-" + g(), o = r > 0 ? i.initialCaption : i.msgSelected.replace(/\{n\}/g, n), s = e(o).text();
            if (c(a) && n > 0) {
                for (var p = 0; n > p; p++)
                    l += "-" + p, t += i.previewGenericTemplate.replace(/\{previewId\}/g, l).replace(/\{content\}/g, a[p]);
                n > 1 && 0 == r && (o = i.msgSelected.replace(/\{n\}/g, n))
            } else {
                if (!(n > 0))
                    return r > 0 ? (i.$caption.html(o), void i.$captionContainer.attr("title", s)) : void 0;
                for (var d = a.split(i.initialDelimiter), p = 0; n > p; p++)
                    l += "-" + p, t += i.previewGenericTemplate.replace(/\{previewId\}/g, l).replace(/\{content\}/g, d[p]);
                n > 1 && 0 == r && (o = i.msgSelected.replace(/\{n\}/g, n))
            }
            ...
            var a, n, r = this, l = r.parseFileType(e), p = r.slug(e.name), c = r.allowedPreviewTypes, m = r.allowedPreviewMimeTypes, u = (e.type, v(l, r.previewTemplates) ? r.previewTemplates[l] : o[l]), h = v(l, r.previewSettings) ? r.previewSettings[l] : s[l], b = parseInt(r.wrapTextLength), y = r.wrapIndicator, C = r.$preview, $ = c.indexOf(l) >= 0, x = d(m) || !d(m) && v(e.type, m);
            if ($ && x) {
                if ("text" == l) {
                    ...
                    n = u.replace(/\{previewId\}/g, t).replace(/\{caption\}/g, p).replace(/\{type\}/g, e.type).replace(/\{width\}/g, h.width).replace(/\{height\}/g, h.height).replace(/\{data\}/g, F) + P
                } else
                    n = u.replace(/\{previewId\}/g, t).replace(/\{caption\}/g, p).replace(/\{type\}/g, e.type).replace(/\{data\}/g, a).replace(/\{width\}/g, h.width).replace(/\{height\}/g, h.height);
                C.append("\n" + n)
            } else
            ...
            function i(p) {
                ...
                if (!d(L) && c(L)) {
                    for (g = 0; g < L.length; g++)
                        C = L[g], v = h[C], $ = void 0 !== v && v(F.type, T), E += d($) ? 0 : $.length;
                    if (0 == E)
                        return y = t.msgInvalidFileType.replace(/\{name\}/g, T).replace(/\{types\}/g, S), void(t.isError = t.showError(y, F, x, p))
                }
                ...
                }, b(F.type, T) ? r.readAsText(F) : r.readAsArrayBuffer(F)) : (t.previewDefault(F, x), a.trigger("fileloaded", [F, x, p]), setTimeout(i(p + 1), 1e3))) : void setTimeout(i(p + 1), 1e3) : (y = t.msgInvalidFileExtension.replace(/\{name\}/g, T).replace(/\{extensions\}/g, j), void(t.isError = t.showError(y, F, x, p)))
            }
            this.reader = new FileReader;
            var t = this, a = t.$element, n = t.$preview, r = t.reader, l = t.$previewContainer, o = t.$previewStatus, s = t.msgLoading, m = t.msgProgress, u = (t.msgSelected, t.previewFileType, parseInt(t.wrapTextLength), t.wrapIndicator, "preview-" + g()), w = e.length, h = t.fileTypeSettings, b = v("text", h) ? h.text : p.text;
            i(0)
        }, slug: function (e) {
            return d(e) ? "" : e.split(/(\\|\/)/g).pop().replace(/[^\w-.\\\/ ]+/g, "")
        }, change: function (i) {
            var t, a = this, n = a.$element, r = a.slug(n.val()), l = 0, o = a.$preview, s = n.get(0).files, p = a.msgSelected, c = d(s) ? 1 : s.length + a.initialPreviewCount;
            if (a.hideFileIcon(), t = void 0 === i.target.files ? i.target && i.target.value ? [{name: i.target.value.replace(/^.+\\/, "")}] : [] : i.target.files, 0 !== t.length) {
                a.resetErrors(), o.html(""), a.overwriteInitial || o.html(a.initialPreviewContent);
                var l = t.length;
                if (a.maxFileCount > 0 && l > a.maxFileCount) {
                    var v = a.msgFilesTooMany.replace(/\{m\}/g, a.maxFileCount).replace(/\{n\}/g, l);
                    return a.isError = a.showError(v, null, null, null), a.$captionContainer.find(".kv-caption-icon").hide(), a.$caption.html(a.msgValidationError), void a.$container.removeClass("file-input-new")
                }
                a.readFiles(s), a.reader = null;
                var m = c > 1 ? p.replace(/\{n\}/g, c) : r;
                a.isError ? (a.$captionContainer.find(".kv-caption-icon").hide(), m = a.msgValidationError) : a.showFileIcon(), a.$caption.html(m), a.$captionContainer.attr("title", e(m).text()), a.$container.removeClass("file-input-new"), n.trigger("fileselect", [c, r])
            }
